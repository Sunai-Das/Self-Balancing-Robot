...
#include <Wire.h> 
#include <I2Cdev.h>
#include <JJ_c.h> // Modified version of the library to work with DMP (see comments inside) 
#define DEBUG 0
...
#define KP_THROTTLE 0.065                    //0.08#define KI_THROTTLE 0.05
...
return (int) &v - (   brkval == 0 ? (int) &   heap_start : (int)     brkval);
...
// Calculate the subperiod. if module <0.25 => subperiod=0, if module < 0.5 => subperiod=1. if module <
0.75 subperiod=2 else subperiod=3
...
{
Serial.print(1000.0/absSpeed);
Serial.print("\t");
Serial.print(absSpeed);
Serial.print("\t"); 
Serial.print(period_m[motor][0]);
Serial.print("-"); 
Serial.print(period_m[motor][1]);
Serial.print("-"); 
Serial.print(period_m[motor][2]);
Serial.print("-");
...
#endif
//target_angle = (target_angle + speedPControl(estimated_speed_filtered,throttle,Kp_thr))/2.0; 
// Some filtering : Average with previous output
//target_angle = target_angle*0.3 + 
speedPIControl(dt,estimated_speed_filtered,throttle,Kp_thr,Ki_thr)*0.7;                // Some filtering
...
setMotorSpeed(1,motor2); 
pushUp_counter=0;
if (OSC.push1)                     // Move arm WITA.Servo(3,750);
else
...
if (distance_sensor < OBSTACLE_DISTANCE_MIN) WITA.Servo(3,2400);
}
...
// Medium loop 40Hz if (loop_counter >= 5)
{

loop_counter = 0;

// if we are in autonomous mode we read the distance sensor if (mode==1)
{
...
// Read battery status readBattery();
#if DEBUG==6
...
digitalWrite(4,HIGH);                 // Disable motors #endif
}
